-- =====================================================================
-- ADIA 0028 - tablero filtrable.
--
-- build_dashboard precalcula el periodo vigente sin filtros: es lo que se
-- carga de entrada, en una sola consulta y sin IA de por medio. En cuanto
-- el usuario filtra por periodo, cargo o texto, esa foto ya no sirve, asi
-- que se recalcula en vivo con la MISMA definicion de cada indicador.
--
-- Los nombres de columna llegan desde el navegador, de modo que se validan
-- contra dataset_columns antes de interpolarse y los valores se meten con
-- %L. La consulta se arma aqui, no en el cliente.
-- =====================================================================
...
begin
  ...
  execute format($q$
    ...
    from (select width_bucket(ln(greatest(%1$I,1)),
                   ln(greatest((select min(%1$I) from %2$s %3$s and %1$I > 0),1)),
...
-- ---------------------------------------------------------------------
-- Opciones de filtro: que columnas se pueden filtrar y con que valores.
-- ---------------------------------------------------------------------
create or replace function public.dataset_filter_options(p_dataset_id bigint, p_limit int default 60)
...
set search_path = public, pg_catalog as $fn$
  select coalesce(jsonb_agg(jsonb_build_object(
           ...
           'values', coalesce((
             select jsonb_agg(jsonb_build_object('value', v.value, 'n', v.n) order by v.n desc)
               from (select value, n from public.dataset_values
                      where dataset_id = p_dataset_id and column_name = c.column_name
                      order by n desc limit p_limit) v), '[]'::jsonb))
         order by c.distinct_count), '[]'::jsonb)
    from public.dataset_columns c
   where c.dataset_id = p_dataset_id and c.role = 'category' and c.is_analyzable;
$fn$;

grant execute on function public.dataset_filter_options(bigint,int) to authenticated;
