-- =====================================================================
-- ADIA 0031 - el tablero agrupa y filtra por la columna normalizada.
-- Con el mapa confirmado, "Desglose por labor_regime" deja de mostrar
-- 1/2/3 y muestra LOSEP/CT/LOEI. El valor crudo sigue intacto en su
-- columna, asi que la trazabilidad no se pierde.
-- =====================================================================
...
begin
  ...
  for k, v in select key, value #>> '{}' from jsonb_each(coalesce(p_filters, '{}'::jsonb)) loop
    ...
    -- Se filtra por la columna NORMALIZADA: el desplegable ofrece "LOSEP",
    -- que en las filas de 2015-06 en adelante esta guardado como "1".
    v_where     := v_where     || format(' and %I::text = %L', app.norm_col(p_dataset_id, k), v);
    v_where_all := v_where_all || format(' and %I::text = %L', app.norm_col(p_dataset_id, k), v);
  ...
  v_cat := app.norm_col(p_dataset_id, v_cat);
  ...
  execute format($q$
    ...
    coalesce((select ', ' || string_agg(quote_ident(app.norm_col(p_dataset_id, column_name)), ', ')
  ...
  loop
    execute format($q$
      ...
      app.norm_col(p_dataset_id, c.column_name),
      case when v_ent is not null then format('count(distinct %I)', v_ent) else 'count(*)' end,
