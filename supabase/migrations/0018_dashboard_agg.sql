-- =====================================================================
-- ADIA 0018 - dos fallos mas en build_dashboard:
--   1) La subconsulta que elige las dos categorias del ranking mezclaba
--      string_agg con ORDER BY sobre una columna no agrupada (42803). El
--      ORDER BY/LIMIT tiene que ir en una subconsulta interna.
--   2) Desfase de uno en el periodo interanual: array_position es 1-based
--      y el indice de un array jsonb es 0-based, asi que el mismo mes del
--      ano anterior es pos-13, no pos-14.
-- =====================================================================
...
declare
  ...
  v_cat text;   -- columna de categoria principal (texto, no jsonb)
begin
  ...
  if v_hasper then
    ...
    v_yoy  := case when jsonb_array_length(v_periods) >= 13
                   then (v_periods->>(array_position(
                          array(select jsonb_array_elements_text(v_periods)), v_def) - 13)) end;
