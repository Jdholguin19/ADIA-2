-- =====================================================================
-- ADIA 0019 - motor de alertas.
--
-- Dos familias: calidad de datos (lo que esta roto en el archivo) y
-- estadisticas/negocio (lo que gerencia deberia mirar). Cada alerta lleva
-- severidad, metrica, impacto en dinero cuando se puede calcular, hasta 20
-- filas de evidencia y el SQL que la genero, para que se pueda auditar.
-- Los umbrales viven en alert_rules: son datos, no codigo.
-- =====================================================================

-- La deteccion acabo viviendo en plpgsql en vez de en plantillas SQL: es
-- mas auditable y evita un mini motor de plantillas fragil. alert_rules
-- conserva los METADATOS configurables (severidad, umbrales, on/off), asi
-- que sql_tpl deja de ser obligatoria.
alter table public.alert_rules alter column sql_tpl drop not null;
...
begin
  ...
  values (p_dataset_id, p_code, r.severity, r.category, p_title, p_detail,
          p_metric, p_impact, coalesce(p_evidence, '[]'::jsonb), p_sql);
end $fn$;

insert into public.alert_rules(code, title_tpl, detail_tpl, severity, category, params, sort_order) values
 ('QUARANTINED_ROWS','Filas en cuarentena','Filas con corrupcion estructural apartadas del analisis.','high','data_quality','{}',10),
 ('PARTIAL_CELLS','Celdas ilegibles','Filas conservadas con alguna celda que no se pudo tipar.','medium','data_quality','{}',20),
 ('DUPLICATE_GRAIN','Registros duplicados','Misma entidad repetida en el mismo periodo: infla las sumas.','high','data_quality','{}',30),
 ('ZERO_OR_NEGATIVE_MONEY','Importes en cero o negativos','Registros con importe principal <= 0.','high','data_quality','{}',40),
 ('DERIVED_INCONSISTENCY','Regla aritmetica incumplida','Una relacion que se cumple en casi todo el archivo falla en unas filas.','high','data_quality','{}',50),
 ('CATEGORY_ENCODING_DRIFT','Cambio de codificacion','Una columna cambia por completo de vocabulario entre dos periodos.','critical','data_quality','{"max_jaccard":0.2}',5),
 ('NEW_CATEGORY_VALUE','Valor nuevo a mitad de serie','Aparece un valor que no existia al principio.','medium','data_quality','{"min_n":20}',60),
 ('HIGH_NULL_RATE','Columna mayormente vacia','Mas del 40% de valores ausentes.','medium','data_quality','{"threshold":0.4}',70),
 ('NEAR_DUPLICATE_LABELS','Etiquetas casi identicas','Variantes del mismo valor que dispersan los agregados.','medium','data_quality','{"similarity":0.8,"ratio":20}',80),
 ('CONSTANT_COLUMN','Columna constante','Un unico valor en todo el archivo.','info','data_quality','{}',200),
 ('WEAK_ENTITY_KEY','Clave de entidad debil','La identidad es un nombre, no un identificador estable.','medium','data_quality','{}',90),
 ('MOJIBAKE','Texto mal codificado','Caracteres de reemplazo: latin-1 leido como utf-8.','medium','data_quality','{}',100),
 ('MISSING_PERIOD','Periodos ausentes','Huecos en la secuencia temporal.','medium','data_quality','{}',110),
 ('PARTIAL_LATEST_PERIOD','Ultimo periodo incompleto','El ultimo periodo tiene muchas menos filas que la mediana.','high','data_quality','{"threshold":0.8}',15),
 ('PAY_OUTLIER_IN_GROUP','Atipicos dentro del mismo cargo','Importes muy alejados de la mediana de su grupo de pares.','high','statistical','{"z":5,"min_group":20}',120),
 ('PERSON_PAY_JUMP','Saltos de importe por persona','Variacion fuerte de un periodo al siguiente.','high','statistical','{"pct":0.3,"abs":200}',130),
 ('HEADCOUNT_JUMP','Salto de plantilla','Variacion mensual muy por encima de lo habitual.','medium','statistical','{"pct":0.03}',140),
 ('PAYROLL_CONCENTRATION','Concentracion del gasto','El 1% mejor pagado concentra buena parte del total.','medium','business','{"threshold":10}',150),
 ('PAY_DISPERSION','Dispersion salarial','Ratio p90/p10 elevado.','low','business','{"threshold":5}',160),
 ('GHOST_EMPLOYEE_CANDIDATE','Posible registro fantasma','Nombres casi identicos cobrando en el mismo periodo.','high','statistical','{"similarity":0.92}',170)
    ...
    if v_sql is not null then
      execute format($q$
        ...
        select coalesce(jsonb_agg(to_jsonb(z) order by (z.z_robusto) desc), '[]'::jsonb) from (
          select b.e entidad, b.g grupo, round(b.v,2) valor, round(s.med,2) mediana_grupo, s.n tam_grupo,
