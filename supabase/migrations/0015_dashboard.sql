-- =====================================================================
-- ADIA 0015 - tablero ejecutivo, precalculado.
--
-- Todo se computa una vez y se guarda en dataset_metrics: el tablero se
-- carga con UNA consulta, es identico para todo el que lo mire y NO pasa
-- por la IA. Los numeros que ve gerencia son deterministas y reproducibles.
-- =====================================================================

-- Columna de dinero principal. El desempate es el ORDINAL, no la suma:
-- annual_remuneration suma 12x mas que monthly_remuneration y ganaria
-- cualquier criterio por magnitud, dejando una "nomina mensual" 12 veces
-- inflada. La convencion universal es listar antes la cifra base.
create or replace function app.primary_money(p_dataset_id bigint)
...
set search_path = public, pg_catalog as $fn$
  select column_name from public.dataset_columns
   where dataset_id = p_dataset_id and role = 'money' and is_analyzable
     and coalesce((stats->>'low_cardinality_numeric')::boolean, false) = false
   order by ordinal
   limit 1;
$fn$;
...
begin
  ...
  execute format($q$
    with b as (select %1$I v from %2$s %3$s where %1$I is not null),
         t as (select count(*) n, sum(v) s from b)
    select jsonb_build_object(
      'top1_share', round(coalesce((select sum(v) from (select v from b order by v desc
                      limit greatest(1, (select ceil(n*0.01) from t)::int)) z)
                      / nullif((select s from t),0) * 100, 0)::numeric, 2),
      'top10_share', round(coalesce((select sum(v) from (select v from b order by v desc
  ...
  execute format($q$
    ...
    from (select width_bucket(ln(greatest(%1$I,1)), ln(greatest((select min(%1$I) from %2$s %3$s where %1$I>0),1)),
                              ln(greatest((select max(%1$I) from %2$s %3$s),2)), 24) b,
                 round(min(%1$I)::numeric,2) lo, round(max(%1$I)::numeric,2) hi, count(*) n
            from %2$s %3$s where %1$I is not null and %1$I > 0 group by 1) z $q$,
